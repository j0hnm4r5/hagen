...
function parseOscRgb(s: string): RGB {
	...
	const to8bit = (hex: string) => Math.round((parseInt(hex, 16) / 0xffff) * 255);
...
function buildPaletteQuery(): string {
	return Array.from({ length: PALETTE_SIZE }, (_, i) => `${ESC}]4;${i};?${BEL}`).join("");
}

/** Response pattern: ESC]4;{index};{rgb}BEL or ESC]4;{index};{rgb}ESC\ */
const OSC_RESPONSE_RE = new RegExp(`\\x1b\\]4;(\\d+);([^\\x07\\x1b]+)(?:\\x07|\\x1b\\\\)`, "g");
...
async function queryPalette(): Promise<Palette> {
	...
	return new Promise((resolve, reject) => {
		...
		const timer = setTimeout(() => {
			cleanup();
			const missing = Array.from({ length: PALETTE_SIZE }, (_, i) => i).filter(
				(i) => !results.has(i)
			);
			reject(
		...
		const onData = (chunk: string) => {
			...
			if (results.size >= PALETTE_SIZE) {
				...
				for (let i = 0; i < PALETTE_SIZE; i++) {
					...
					palette[name] = { index: i, rgb: parseOscRgb(value) };
				}
				resolve(palette);
			}
		};
...
async function main() {
	try {
		...
		if (err instanceof UnsupportedTerminalError) {
			console.error("Unsupported terminal:", err.message);
		} else {
			console.error(err);
		}
		process.exitCode = 1;
